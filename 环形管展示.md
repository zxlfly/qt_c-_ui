...
layout->addWidget(widget);  // 必须加入布局！
...
| `setRotationOffset(double degrees)` | 设置旋转偏移角度（度数），大管+小管整体旋转，倒三角位置不变 |
...
| `rotationOffsetChanged(double degrees)` | 旋转偏移变化时发射 |
...
控件大小变化时自动重绘，所有尺寸基于 `outerR` 的比例计算。最小尺寸 200×200。

### 布局特性

- `sizeHint()` 返回 `QSize(400, 400)`
- `sizePolicy` 为 `Expanding` × `Expanding`，在布局中自动伸展
- `minimumSize` 为 `200 × 200`，支持多控件共存
- 设置了 `WA_OpaquePaintEvent` + `setAutoFillBackground`，控件始终有可见背景

### 多控件使用

支持在同一个页面中放置多个 CircleDisplayWidget 实例：
...
for (int i = 0; i < 4; ++i) {
    auto *gb = new QGroupBox(QString("管组 %1").arg(QChar('A' + i)), this);
...
### CircleDemoPage（环形管展示）

2×2 网格放 4 个 CircleDisplayWidget 实例（管组 A/B/C/D），每个初始随机文字：
- 「全部随机更新」：同时更新 4 个控件的 12 个小管文字
- 「全部重置」：同时重置 4 个控件的文字为"名称"
...
| `circledemopage.h/cpp` | 演示页面（4实例，随机更新、重置按钮） |
| `circleselectdemopage.h/cpp` | 点击选择演示页面（4实例，弹出列表选择后更新管内文字） |
| `circlerotatedemopage.h/cpp` | 旋转切换演示页面（4实例，动画旋转，倒三角指向小管） |

---

## 注意事项

1. **必须加入布局**：创建 CircleDisplayWidget 后必须调用 `layout->addWidget()`，否则控件浮在 (0,0) 位置拦截鼠标事件
2. **多控件共用**：多个 CircleDisplayWidget 实例互不影响，各自维护独立的数据
3. **最小尺寸**：200×200，多控件在网格布局中可正常分配空间
